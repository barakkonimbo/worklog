...
מסנכרן בלוקי-זמן לפי פרויקט + אירוע "סיכום יום" ל**יומן ייעודי "Work Journal"** (לא נוגע באירועים אמיתיים) — **mirror מתמשך** (פרטי התזמון למטה). דורש סטאפ OAuth חד-פעמי:
...
הסנכרון רץ בכל סגירת סשן (היומן = **mirror מתמשך**, מתעדכן גם אם עבדת אחרי 20:30) וגם בריצת ה-20:30 (שמחדשת את אירוע הסיכום). כיבוי: `worklog-config.js calendar off`.
