...
## עדכון גרסה
משתפים zip חדש, החבר פורס מעליו, ואז מריץ **`/worklog update`** (v0.8.0+) — בודק לפי **content-manifest**
(לא רק מספר גרסה, אז גם hotfix נתפס), מסביר מה השתנה (מ-`upgrade-notes.json`), ומסמן אם נדרשת התייחסות.
חלופה זהה בתוצאה: `/work-journal-setup` שוב. ההתקנה **אידמפוטנטית** — מחליפה את החלקים שלנו בלי לשכפל,
ושומרת hooks אחרים (GSD וכו') ואת ה-creds המוצפנים. (מי שלפני 0.8.0 — עדכון ראשון דרך `/work-journal-setup`.)

**תחזוקת `upgrade-notes.json`:** בכל bump הוסף רשומה `"<version>": { "summary": "<שורה>", "action": null }`.
קבע `action: { "required": true, "text": "..." }` רק אם המשתמש **חייב** לעשות משהו (נדיר, למשל שינוי פורמט-cred).
