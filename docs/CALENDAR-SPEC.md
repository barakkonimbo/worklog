...
> סטטוס: **נבנה ואומת חי** (v0.7.0, 2026-06-04; **v0.7.3** — mirror מתמשך; **v0.7.4** — fallback מבוסס-רשומות + תיאור HTML; **v0.7.6** — בלוקים מבוססי-שכבת-פעילות; **v0.8.0** — `parseEntries` עבר לפרסר-הרשומות הסובלני המשותף `lib.parseEntryLine`, עמיד ל-reformat של ה-markdown; **v0.8.1** — קיבוץ בלוקי-פעילות **פר-פרויקט** סובלני-להשתלבות; **v0.8.2** — `push`/`unpush`/auto-mirror ליומן המשתמש) — `--setup` / `--test` / `--sync` / `--push` / `--unpush` / אידמפוטנטיות ✅.
...
## שכבת פעילות — בלוקים מבוססי-prompt (v0.7.6, **המסלול הראשי**)
**התובנה:** עיגון-סשנים גס מדי (מרווח אחד לכל הסשן) ונקודות-מפתח דלילות מדי. הפתרון — **שתי שכבות נפרדות**:
- 🕐 **שכבת פעילות:** hook `UserPromptSubmit` (`worklog-prompt.js`) חותם **זמן+פרויקט בלבד** (ללא תוכן) לפני כל מענה → `.sessions/<date>.activity.jsonl`. זו שכבת הזמנים הצפופה שמגדירה בלוקים מדויקים.
- 📝 **שכבת תוכן:** רשומות נקודות-המפתח (`YYYY-MM-DD.md`) — מזינות את סיכום ה-AI. **נפרדת** מהפעילות כדי לא להציף את הסיכום ברעש.

**פרמטרים (ניתנים-לכוונון ב-config):** `activityGapMinutes=30` (פיצול), `tailMinutes=10` (זנב).

**חישוב (`computeBlocks`, כשיש חותמות-פעילות):**
1. **קיבוץ פר-פרויקט (v0.8.1):** לכל פרויקט בנפרד, חותמות שלו (בסדר זמן) שמרווחן זו-מזו ≤ `activityGap` (30ד׳) = streak אחד. **סובלני להשתלבות:** קפיצה לפרויקט אחר באמצע **לא** שוברת את הרצף, כל עוד הפרויקט חוזר תוך 30ד׳ — כך `turk … someSkills … turk` בטווח 30ד׳ נשאר **בלוק turk אחד** (+ בלוק someSkills), במקום להתפצל לבלוק לכל חותמת. (לפני v0.8.1 הקיבוץ היה רציף-גלובלי → השתלבות פיצצה כל פרויקט להמון בלוקים זעירים — באג שתוקן.)
2. בלוק לכל streak: התחלה = חותמת ראשונה; סוף = **חותמת אחרונה + `tail`** (10ד׳).
3. **ללא clamp בין-פרויקטי (v0.8.1):** פרויקטים שונים עצמאיים → הבלוקים שלהם **רשאים לחפוף** כשהעבודה הייתה משולבת ("לכל פרויקט הקובייה שלו"). רצפי אותו-פרויקט תמיד >30ד׳ זה מזה, אז הזנב (10ד׳ < 30) לעולם לא יוצר חפיפה תוך-פרויקטית. חותמת בודדת → בלוק בן ~10ד׳.
4. **notes:** רשומות-תוכן אותו-פרויקט שנופלות בחלון הבלוק. רשומה שלא מכוסה ע"י אף בלוק → בלוק **fallback** (כמו במסלול legacy) — עבודה מתועדת ידנית לא הולכת לאיבוד.
5. **שני המספרים אינם חופפים:** 30 = *מתי מפצלים*; 10 = *כמה הבלוק נמשך אחרי החותמת*. **אין Stop hook** (חישוב טהור על החותמות; אפס רשומות נוספות).

**דיוק מול קירוב:** רצף עבודה צפוף נתפס היטב; שתיקה > 30ד׳ מפצלת (גם אם קלוד עבד ברקע על משימה ארוכה — קירוב מקובל, אושר ע"י המשתמש). כשאין חותמות (ימים שלפני v0.7.6 / hook לא ירה) → המסלול הישן למטה.

## מירור ליומן המשתמש — push / unpush / autoPush (v0.8.2)
ברירת המחדל כותבת ל**יומן "Work Journal" הפרטי בלבד**. בנוסף, אפשר **למרר** את היום ליומן אחר של המשתמש (ברירת מחדל `primary` = היומן הראשי שלו):
- **`--push [date]`** = `syncDay` עם `calId = pushCalendarId` — **העתק מלא 1:1** (אותם בלוקים + סיכום, ללא סינון), **בנוסף** ל-Work Journal (לא מחליף). אידמפוטנטי דרך אותו תג `worklog=<date>` (לחיצה חוזרת מחליפה). בלי claude.
- **`--unpush [date]`** = מחיקת אירועי-`worklog` של אותו יום מהיעד (retract). מחיקה תחומה-לתג → לעולם לא נוגעת בפגישות אמיתיות.
- **`autoPush` (opt-in, כבוי):** בריצת רצף סיכום-היום (`worklog-summary` כש-`want.deliver`) מתבצע גם `--push`. לא רציף (לא חושף WIP).
- **יעד:** `pushCalendarId` (ברירת מחדל `'primary'`, פר-משתמש). בחירה: `--push-setup` (תפריט אינטראקטיבי) או `--list-calendars` + `worklog-config.js push.calendar <id|primary>`. ה-OAuth scope המלא מספיק לכל יומני המשתמש.

## אלגוריתם הבלוקים — מעוגן-סשנים, גזום-פערים (node, 0 טוקני AI — **fallback ל-legacy**)
...
**מגבלה מובנית:** נלכדת רק עבודה **מול Claude** (חותמת-prompt, רשומה, ו/או סשן); עבודה מחוץ ל-Claude שלא תועדה — לא ביומן ממילא. (לפני v0.7.4 נדרש סשן שעוטף את הרשומה; v0.7.4 הסיר את התנאי דרך fallback; v0.7.6 לוכד כל prompt דרך שכבת-הפעילות — כך גם רצף שלא נרשמו בו נקודות-מפתח נתפס.)
