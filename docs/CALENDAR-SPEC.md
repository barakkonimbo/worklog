...
> סטטוס: **נבנה ואומת חי** (v0.7.0, 2026-06-04; **v0.7.3** — mirror מתמשך; **v0.7.4** — fallback מבוסס-רשומות + תיאור HTML; **v0.7.6** — בלוקים מבוססי-שכבת-פעילות; **v0.8.0** — `parseEntries` עבר לפרסר-הרשומות הסובלני המשותף `lib.parseEntryLine`, עמיד ל-reformat של ה-markdown) — `--setup` / `--test` / `--sync` / אידמפוטנטיות ✅.
...
1. מיין חותמות; קבץ חותמות **אותו-פרויקט** שמרווחן ≤ `activityGap` (30ד׳) ל-streak. מרווח > 30 → בלוק חדש; מעבר פרויקט → בלוק חדש.
2. בלוק לכל streak: התחלה = חותמת ראשונה; סוף = **חותמת אחרונה + `tail`** (10ד׳).
3. **Clamp:** אם הזנב גולש לתוך הבלוק הבא → קוצר לתחילתו (לעולם **אין חפיפה**, גם במעבר-פרויקט מהיר). חותמת בודדת → בלוק בן ~10ד׳.
